...
\lstdefinestyle{mystyle}{
    ...
    emph={graph,objectives,visited,queue,cost,node,angle,obj,e,h,new_cost,next}, emphstyle=\color{teal}
}
...
The described algorithm yield an optimal solution 
...
The refined process consists in adding more angle
...
in the space directly surrounding the angle chosen in te first
solution.
For example, if in the first iteration of a solution we pass trough node A with
an angle of 30 degrees, in the second iteration we would add angles 20, 25, 35, and 40
to node A's arriving angle possibility.
This step is repeated twice, with it yielding a average improvement
...
but the gains gets progressively smaller.
...
with their position of the map, as well as the 
...
as is a sub-case of sinister TSP \cite{sinister_tsp}.
...
of the solution-space in order to guaranteed an optimal outcome.
...
to find ``good enough'' solutions in a reasonable time,
...
possible solutions, as this guaranteed the optimal solution
while still having acceptable computational times.

\section{Results}
\label{sec:results}

In this section we will sow how the algorithm performs under various scenarios with a particular
...
All tests have being executed on a six-core twelve-threads ryzen 5 5600.
...
Different part of the program were measured to understand
where most of the time is spent. In particular
the measured part were:
\begin{itemize}
  ...
  \item \textbf{[Ref]:} Refinement of the best solution (\ref{sec:refinement})
\end{itemize}
...
However this speed improvement comes at the expenses of the quality of the 
...
We can see an improvement of approximately 6x when the process is compute-bounded
(such as for example the graph-search part of the sampling approach). Up to a 9x 
improvement when the process is memory-bounded (such as for example the trajectory
planning step).
At first it may seam counter intuitive that a 6 core CPU manage to get a greater
than 6x improvement in performance when using multithreading, however this 
is simply due to the fact that the process is memory bounded, and
the CPU is therefore able to take advantage of the 12 threads (in particular the
memory bounded part is doing collision detection by accessing the 
$1000 \times 1000$ grid \ref{sec:cd})


\subsection{Trajectory quality}

In the two figure \ref{fig:sampling_path} and \ref{fig:combinatorial_path} we can see 
the same map solved by using a sampling based,a nd a combinatorial approach respectively.
It is easy to see how the combinatorial approach is worse than the sampling one.
This is intuitive as the sampling one, although is slower, is able to generate much
larger graphs, which gives the graph solver more flexibility when it tomes
to find a good path.
...
\subsection{Collision avoidance}
The proposed algorithm should guaranteed that the found paths are collision free
...
the desired path by a significant margin (approximately 0.3 unit of distance)
especially when searing for prolonged period of time.
This was addressed by increasing the turning radius, and adjusting the margin
to account for controller drift.
No other crash was observed after this fixes.

\subsection{Conclusions}

In general we can sey that when comparing sampling based and combinatorial strategies to build
the planning graph the sampling one are the better option for our scenario.

They are a bit slower than the other option when it comes to computation time, but they
more than make up for it in the goodness of the trajectory.
...
slower in the computation time, but resulted in a path that was 16 seconds
...
On the other hand the CPU used to run the tests is a but outdated at this point, and
I believe that simply using a more modern one could bring the execution time
fro the sampling strategy below one second.
...
In general we can note how when there is no time pressure the there is not a big differences
...
\newline
\textit{rescue\_planner\/src\/util\/constants.hpp}
\newline
...
The code has been written entirely by me expect the part to generate charts (that is AI-generated)
